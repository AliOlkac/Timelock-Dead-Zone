...
public class silahdeneme : MonoBehaviour
{
    ...
    {
        crossHair.color = Color.red; // Crosshair rengini k�rm�z� yap
        StartCoroutine(ResetCrossHairColor()); // Bir s�re sonra rengi beyaza d�nd�r
    }

    // Ni�angah rengini tekrar beyaza d�nd�ren Coroutine
    private IEnumerator ResetCrossHairColor()
    {
        yield return new WaitForSeconds(.2f); // 1 saniye bekle
        crossHair.color = Color.white; // Ni�angah rengini tekrar beyaz yap
    }
    void Reload()
    {
        if (mag > 0)
